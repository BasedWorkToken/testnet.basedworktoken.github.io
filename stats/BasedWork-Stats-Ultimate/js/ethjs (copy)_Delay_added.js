...
HttpProvider.prototype.sendAsync = function (payload, callback) {
  ...
  // Add a queue if it doesn't exist
  ...
  // Enqueue the request
  this._requestQueue.push({ payload, callback });
 ...
 */
HttpProvider.prototype.processQueue = function () {
  ...
  const processNextRequest = () => {
    ...
    const { payload, callback } = this._requestQueue.shift();
    ...
    request.onreadystatechange = () => {
      if (request.readyState === 4 && request.timeout !== 1) {
        ...
        try {
          ...
          error = invalidResponseError(request, this.host);
        }
        callback(error, result);

        // Schedule next request with a delay
        setTimeout(processNextRequest, 200); // 1 second delay between requests
      }
    };

    request.ontimeout = () => {
      callback('[ethjs-provider-http] CONNECTION TIMEOUT: http request timeout after ' + this.timeout + ' ms.', null);
      
      // Still process next request
    ...
    try {
      ...
      callback('[ethjs-provider-http] CONNECTION ERROR: Couldn\'t connect to node \'' + this.host + '\': ' + JSON.stringify(error, null, 2), null);
